    ...
    sensorFilter.forEach(item => {
      let matrix4 = new THREE.Matrix4().compose(
        ...
        sensorList[0]["rotation_type"] === "euler"
          ? new THREE.Quaternion().setFromEuler(new THREE.Euler(parseFloat(item.roll), parseFloat(item.pitch), parseFloat(item.yaw)))
          : new THREE.Quaternion(parseFloat(item.q_x), parseFloat(item.q_y), parseFloat(item.q_z), parseFloat(item.q_w)),
        ...
        <group rotation-z={Math.PI / 2} scale={[0.1,0.1,0.1]} matrixAutoUpdate={false} matrix={sensorViewerMatrix[prop]} key={prop} ref={(ref) => meshRefs.current[prop] = ref}>
          ...
          <Html position={[0, 0, 0]} center >
            <div
              ...
              onClick={() => meshRefs.current[prop].visible = !meshRefs.current[prop].visible}
